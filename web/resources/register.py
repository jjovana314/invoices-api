...
class Register(Resource):
    ...
    def post(self):
        list_posted_data = request.get_json()
        global liability
        ...
        result_dict = validate_num_of_invoices(list_posted_data, result_dict)
        if len(result_dict.values()) == 0:
            for curr_invoice in list_posted_data:
                message, code = validate_schema_caller(
                    curr_invoice, "schema_register"
                )
                if code != HTTPStatus.OK:
                    # update liability_error dictionary
                    liability_error["SchemaError"] = message
                    liability_error["InvoiceNumber"].append(
                        curr_invoice.get("InvoiceNumber")
                    )
                else:
                    # if data is valid
                    validate_date_caller(curr_invoice)
                    liability["InvoiceNumber"].append(
                        curr_invoice.get("InvoiceNumber")
                    )
                    idf = generate_idf(curr_invoice["InvoiceNumber"])
                    idf_list.append(idf)
                    if invoices.find_one({"invoiceId": idf}):
                        return jsonify(
                            {
...
def validate_num_of_invoices(list_posted_data: list, result_dict: dict) -> dict:
    global max_invoices_request
    global liability_error
    if len(list_posted_data) > max_invoices_request:
